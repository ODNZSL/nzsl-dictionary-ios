...
class SearchViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    override func loadView() {
        ...
        let modeSwitch = UISegmentedControl(items: ["Abc", UIImage(named: "hands")!])
...
//        subsequent_keyboard = true
    ...
    func selectSearchMode(sender: UISegmentedControl) {
        // TODO: should we use `sender` rather than the property we assume it will be?
        switch modeSwitch.selectedSegmentIndex { // TODO: this is iOS 8.0 and later
        case 0:
            searchBar.text = ""
...
//            self.collectionView(<#T##collectionView: UICollectionView##UICollectionView#>, didDeselectItemAtIndexPath: <#T##NSIndexPath#>)
            NSIndexPath()
        }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            ...
            var iv: UIImageView = UIImageView(frame: CGRectMake(0, 2, tableView.rowHeight * 2, tableView.rowHeight - 4))
            iv.contentMode = .ScaleAspectFit
            cell.accessoryView = iv
        }
        var e: DictEntry = searchResults[indexPath.row]
        cell.textLabel.text = e.gloss
        cell.detailTextLabel.text = e.minor
        var iv: UIImageView = cell.accessoryView as! UIImageView
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        var entry: DictEntry = searchResults[indexPath.row]
    ...
    func collectionView(collectionView: UICollectionView, didDeselectItemAtIndexPath indexPath: NSIndexPath) {
        ...
        // TODO indexPathsForSelectedItems is iOS 8+ only ???
        let handshape: [NSIndexPath]? = handshapeSelector.indexPathsForSelectedItems()
        let location: [NSIndexPath]? = locationSelector.indexPathsForSelectedItems()

        let targetHandshape: NSIndexPath?
        if handshape!.count > 0 {
            targetHandshape = HandShapes[handshape![0].row]
        } else {
            targetHandshape = nil
        }

        let loc = location.count ? Locations[((NSIndexPath *)location[0]).row][0] : nil

        searchResults = dict.searchHandshape(targetHandshape, location: loc)
