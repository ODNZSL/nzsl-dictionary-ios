...
class DiagramView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        detailView = DetailView(frame: CGRect(x: viewPadding, y: viewPadding, width: self.bounds.size.width - (viewPadding * 2), height: DetailView.height))
        detailView.autoresizingMask = UIView.AutoresizingMask.flexibleWidth

        self.addSubview(detailView)

        imageView = UIImageView(frame: CGRect(x: viewPadding, y: detailView.frame.maxY, width: self.bounds.size.width - (viewPadding * 2), height: self.bounds.size.height-detailView.frame.maxY))
